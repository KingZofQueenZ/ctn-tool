...
export class NewsService {
  ...
  create(news: News): Observable<News> {
    return this.http.post<News>('/api/news', news, {headers: headers, responseType: 'text' as 'json'});
  }
  ...
  update(news: News): Observable<any> {
    return this.http.put('/api/news/' + news._id, news, {headers: headers, responseType: 'text' as 'json'});
  }

  // DELETE /evnewsents/{news-id}
  delete(_id: string): Observable<News> {
    return this.http.delete<News>('/api/news/' + _id, {headers: headers, responseType: 'text' as 'json'});
  }
